...
$cekPMS = "PMS$tampilfaktur[no_faktur_sup]";
if ($cekpmb == $cekPMS) {
...
mysqli_query($con, "INSERT INTO cash_sup VALUES(NULL,'PMS$tampiltrans[no]','$tgl[tanggal_transaksi]','$_POST[level]',$tampilkan[total_harga],0,0,$bayar,$sisa,'TIDAK','$_POST[ket]')");
...
$qryhutang  = mysqli_query($con, "SELECT MAX(CONCAT(LPAD((RIGHT((no_piutang_sup),9)+1),9,'0')))FROM piutang_sup");
$qryhutang2 = mysqli_query($con, "SELECT MIN(CONCAT(LPAD((RIGHT((no_piutang_sup),9)),9,'0')))FROM piutang_sup"); 
...
mysqli_query($con, "insert into piutang_sup values('PIS$kodeautohtg','PMS$tampiltrans[no]','$tgl[tanggal_transaksi]',$tampilkan[total_harga],$bayar,$sisa)");
}else {
//INPUT CASH
$maks2=mysqli_query($con, "select MAX(CONCAT(LPAD((RIGHT((no_transaksi_sup),9)),9,'0'))) as no from pembelian");
$tampiltrans=mysqli_fetch_array($maks2);
...
$tampilkan=mysqli_fetch_array($kuery);
$tgl=mysqli_fetch_array(mysqli_query($con, "select DISTINCT tanggal_transaksi from pembelian where no_transaksi_sup = 'PMS$tampiltrans[no]'"));
mysqli_query($con, "INSERT INTO cash_sup VALUES(NULL,'PMS$tampiltrans[no]','$tgl[tanggal_transaksi]','$_POST[level]',$tampilkan[total_harga],$bayar,$kembalian,0,0,'YA','$_POST[ket]')");
mysqli_query($con, "INSERT INTO cashflow VALUES (NULL,'PMS$tampiltrans[no]','$tgl[tanggal_transaksi]','Pengeluaran',$tampilkan[total_harga],'-')");
}
}
//************************************//
...
$sql_limit = "select * from pembelian,produk where pembelian.kode_barang = produk.id_barang and pembelian.no_transaksi_sup = 'PMS$tmpmaks[no]'";
$query=mysqli_query($con, $sql_limit);
$row = mysqli_fetch_array(mysqli_query($con, "select tanggal_transaksi from pembelian where no_transaksi_sup = 'PMS$tmpmaks[no]'"));
$row2 = mysqli_fetch_array(mysqli_query($con, "select nama from pembelian,supplier where pembelian.id_supplier = supplier.id_supplier and pembelian.no_transaksi_sup = 'PMS$tmpmaks[no]'"));
...
<h3>Invoice</h3>
<table style='width:700px; font-size:10pt; font-family:arial' border = '0'>
<tr><td align='left' style='padding-left:500px'>Kepada Yth, </br>$row2[nama]</td></tr>
<tr><td align='left'>No Invoice: PMS$tmpmaks[no]</td></tr>
</table>
...
$sqlket = mysqli_fetch_array(mysqli_query($con, "select * from cash_sup where no_faktur_sup = 'PMS$tampil[no]'"));

echo"</table></center></br>";
echo"<center><table style='width:700px; font-size:10pt;'>";
$kuery2=mysqli_query($con, "select kasir from cash_sup where no_faktur_sup = 'PMS$tmpmaks[no]'");
