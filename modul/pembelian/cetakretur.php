...
$sql_limit = "SELECT * from pembelian where no_transaksi_sup = '$_GET[no_faktur_sup]'";
$query=mysqli_query($con, $sql_limit);
$tampil=mysqli_fetch_array($query);


//************************************//
$sql_limit = "SELECT * from pembelian,produk where pembelian.kode_barang = produk.id_barang and pembelian.no_transaksi_sup = '$_GET[no_faktur_sup]'";
$query=mysqli_query($con, $sql_limit);
$row = mysqli_fetch_array(mysqli_query($con, "SELECT tanggal_transaksi from pembelian where no_transaksi_sup = '$_GET[no_faktur_sup]'"));
$row2 = mysqli_fetch_array(mysqli_query($con, "SELECT nama from pembelian,supplier where pembelian.id_supplier = supplier.id_supplier and pembelian.no_transaksi_sup = '$_GET[no_faktur_sup]'"));
...
<tr><td align='left'>No Invoice: $_GET[no_faktur_sup]</td></tr>
</table>
...
$kuery=mysqli_query($con, "SELECT SUM(total_harga) as total_harga from pembelian where no_transaksi_sup = '$_GET[no_faktur_sup]'");
$tampilkan=mysqli_fetch_array($kuery);
echo"<tr><td colspan = '4'><div style='text-align:right'><b>Total Yang Harus Di Bayar Adalah : </b></div></td><td style='text-align:right'>Rp".number_format($tampilkan['total_harga'],2,',','.')."</td></tr>";
echo "<tr><td colspan = '5'><div style='text-align:right'><b>Terbilang :</b>".ucwords(Terbilang($tampilkan['total_harga']))." Rupiah</div></td></tr>";
$kuery2=mysqli_fetch_array(mysqli_query("SELECT cash,kembali,dp,sisa from cash_sup where no_faktur_sup = '$_GET[no_faktur_sup]'"));
...
$sqlket = mysqli_fetch_array(mysqli_query($con, "SELECT * from cash_sup where no_faktur_sup = '$_GET[no_faktur_sup]'"));
echo"</table></br>";
echo"<table style='width:700px; font-size:10pt;'>";
//$kuery2=mysql_query("SELECT kasir from cash where no_faktur_sup = '$_GET[no_faktur_sup]'");
//$tampilkan2=mysql_fetch_array($kuery2);
